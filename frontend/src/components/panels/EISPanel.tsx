    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-3 py-1.5 bg-gray-100 border-b border-gray-300 text-xs text-gray-900 shrink-0">
        <div className="flex rounded overflow-hidden border border-gray-300">
          {(["nyquist", "bode", "complex_cap"] as View[]).map(v => (
            <button key={v} onClick={() => setView(v)}
              className={`px-2 py-0.5 text-xs transition-colors ${view === v ? "bg-forest-600 text-white" : "text-gray-700 hover:bg-gray-200"}`}>
        ...
        <label className="flex items-center gap-1 shrink-0">
          <span className="text-[10px] text-gray-600">Fit</span>
          <select value={fitModel} onChange={e => setFitModel(e.target.value)}
                  className="bg-white border border-gray-300 rounded px-1 py-0.5 text-gray-900 text-[10px]">
        ...
        {axesOpen && (
          <div className="w-full flex flex-wrap items-center gap-x-2 gap-y-1 pt-1 border-t border-gray-200">
            ...
            <AxisInput value={xMin} onChange={setXMin} placeholder="min" className="w-16 bg-white border border-gray-300 rounded px-1 py-0.5 text-gray-900" />
            <span className="text-gray-400">–</span>
            <AxisInput value={xMax} onChange={setXMax} placeholder="max" className="w-16 bg-white border border-gray-300 rounded px-1 py-0.5 text-gray-900" />
            ...
            <AxisInput value={yMin} onChange={setYMin} placeholder="min" className="w-16 bg-white border border-gray-300 rounded px-1 py-0.5 text-gray-900" />
            <span className="text-gray-400">–</span>
            <AxisInput value={yMax} onChange={setYMax} placeholder="max" className="w-16 bg-white border border-gray-300 rounded px-1 py-0.5 text-gray-900" />
            <label className="flex items-center gap-1 cursor-pointer text-gray-600">
      ...
      {eis && (
        <div className="flex flex-wrap items-center gap-1.5 px-3 py-1 bg-forest-900/50 border-b border-forest-700/30 shrink-0">
          ...
          <Tooltip content="Show formulas and definitions">
            <button onClick={() => setShowInfo(true)}
                    className="ml-auto text-[10px] text-forest-500 hover:text-forest-300 border border-forest-700 rounded-full w-4 h-4 flex items-center justify-center transition-colors cursor-pointer shrink-0">
              ?
            </button>
          </Tooltip>
        </div>
